...
Menu option whose caller owns exclusive checked state.

```tsx
import { DropdownMenuRadioItem } from 'jakeui';
```
...
- Render inside a menu. Caller owns exclusive selection and checked state; onSelect invokes the choice.
